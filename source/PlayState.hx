...
class PlayState extends MusicBeatState
{
	...
	public static var daIconOffset:Array<Float>;
	...
	override public function create()
	{
		...
		//vcrStuff.setDistortion(false);
		//vcrStuff.setNoise(true);
		//vcrStuff.setVignette(true);
		//vcrStuff.setVignetteMoving(true);

		//var funny:shaders.VCRdistortion;
		//funny = new shaders.VCRdistortion();

		//camGame.setFilters([new ShaderFilter(vcrStuff.shader)]);
  ...
  public function clearShaderFromCamera(cam:String){
	...
	override public function update(elapsed:Float)
	{
		...
		daIconOffset = [iconP1.x, iconP1.y];
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead && SONG.song != 'gift')
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				DeathError();
				openSubState(new GameOverSubstate(camFollowPos.x, camFollowPos.y));
		...
		else if (((skipHealthCheck && instakillOnMiss) || health == 1.99) && !practiceMode && !isDead && SONG.song == 'gift')
		{
			var ret:Dynamic = callOnLuas('onGameOver', []);
			if(ret != FunkinLua.Function_Stop) {
				boyfriend.stunned = true;
				...
				if (SONG.song.toLowerCase() == 'think')
					canDo = true;
